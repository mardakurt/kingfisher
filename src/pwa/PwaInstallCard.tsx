...
export function PwaInstallCard() {
  ...
  if (state.installed) {
    return (
      <div className="pwa-install-card" data-state="installed">
        ...
        <p>
          You can open Kingfisher from your applications or home screen. Removing
          the app from the system will not delete your Studies or Repertoire —
          those live in the browser&apos;s storage for this origin, not in the
          app bundle.
        </p>
      </div>
    );
  ...
  if (!state.available) {
    return (
      <div className="pwa-install-card" data-state="hint">
        ...
        <p>
          Chromium-based browsers can install Kingfisher Studio as an application
          from the address bar (the <em>Install</em> button or the page-info
          menu). Safari and Firefox expose the same option through{' '}
          <em>File &rsaquo; Add to Dock</em> or{' '}
          <em>Share &rsaquo; Add to Home Screen</em>, depending on the platform.
        </p>
        <p>
          The browser only offers the install once per site visit; if you have
          already dismissed it, reload the page to ask again.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="pwa-install-card" data-state="available">
      ...
      <p>
        Add Kingfisher Studio to your applications so you can reopen it like a
        native app — same origin, same Studies, same Repertoire, no second copy
