...
import { materialMatchesCounts, type MaterialQuery, type MaterialSide } from './material-query';
import { findRoute, type Route, type RouteMove, type RoutePiece } from './route';

export const LINE_INDEX_VERSION = 1;

export interface MaterialRun {
  /** Index of the first position of the run; position 0 is the start. */
  readonly start: number;
  readonly w: MaterialSide;
  readonly b: MaterialSide;
}
...
export interface LineIndexParts {
  /** The ply of the start position (0 for a game from the initial position). */
  readonly firstPly: number;
  /** Positions in the line: the moves and one more. */
  readonly positions: number;
  readonly runs: readonly MaterialRun[];
  readonly moves: readonly IndexedMove[];
  /** The theme definitions' version these first plies were computed with. */
  readonly themesVersion: number;
  /** Theme id → the first ply it held for two positions (or at the end). */
  readonly themes: ReadonlyMap<string, number>;
}

const PIECES: readonly (RoutePiece | null)[] = [null, 'p', 'n', 'b', 'r', 'q', 'k'];
const SIDE_KEYS = ['q', 'r', 'b', 'n', 'p'] as const;

/** a1 = 0 … h1 = 7, a2 = 8 … h8 = 63. */
export const squareIndex = (square: string): number =>
...
export function encodeLineIndex(parts: LineIndexParts): Uint8Array {
  const themes = [...parts.themes.entries()].map(
    ([id, ply]) => [new TextEncoder().encode(id.slice(0, 64)), ply] as const,
  );
  const size =
    1 +
    2 +
...
export function decodeLineIndex(bytes: Uint8Array): LineIndexParts | null {
  ...
  for (let index = 0; index < runCount; index += 1) {
    ...
    const side = (offset: number): MaterialSide => ({
      q: counts[offset]!,
      r: counts[offset + 1]!,
      b: counts[offset + 2]!,
      n: counts[offset + 3]!,
  ...
  for (let index = 0; index < moveCount; index += 1) {
    ...
    moves.push({
      ...
      mover: mover
        ? { color: ((value >> 4) & 1) === 1 ? 'b' : 'w', type: PIECES[mover] as RoutePiece }
        : null,
...
export interface IndexedQuery {
  ...
  /** The theme definitions' version of the asker; an index built with others cannot answer. */
  readonly themesVersion: number;
}

export type IndexedAnswer =
  | { readonly kind: 'hit'; readonly ply: number }
  | { readonly kind: 'miss' }
  /** The index cannot answer this question; replay the game instead. */
 ...
 */
export function scanLineIndex(line: LineIndexParts, query: IndexedQuery): IndexedAnswer {
  ...
  if (query.route) {
    const moves: RouteMove[] = line.moves.map((move, index) => ({
