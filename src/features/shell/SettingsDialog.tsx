...
export function SettingsDialog() {
  ...
  */
  ...
  /*
    Filter the left rail by section label and by the keywords the section's
    cards carry. The two are joined: a section is shown if its label OR any
    card keyword in it OR any card label in it OR any description on a card
    matches the query. This is the same `searchSettings` index the
    preference-search inside each section uses, restricted to one section.
  */
  ...
  return (
    ...
    >
      ...
      >
        {/*
          Phase 57: a section-filter input above the rail. The input is
          a single text field with a clear (×) button; typing narrows
          the rail to sections whose label, cards, or card descriptions
          match the query. Empty input restores the full list.
        */}
        <input
          type="search"
          value={filter}
