...
/**
 * A flipped board does not travel.
 *
 * Phase 55 animated each piece's `translate(x%, y%)` to its mirrored square,
 * and thirty-two pieces converging on the centre of the board on the way
 * read as a tablecloth being flapped. It was replaced with a `rotateY(180deg)`
 * of the whole board, which is one motion instead of thirty-two — but a
 * rotation about the vertical axis foreshortens the board to a line and back,
 * so every piece still rushes to the middle and out again, now with the
 * squares. Two attempts, the same complaint.
 *
 * Phase 72: nothing moves. Pieces and squares snap to their new places with
 * no per-piece transition, and the board fades through a short dip in opacity
 * so the change registers as one event rather than a jump cut. That is what
 * the published boards do (Lichess and Chess.com flip instantly); the dip is
 * the one concession to the eye. Reduced motion removes it through the global
 * stylesheet's rule on transitions.
 */
...
export function Chessboard({
  ...
  // True for FLIP_DURATION_MS after the orientation changes: the board dips
  // and the pieces skip their translate transition. See FLIP_DURATION_MS.
  ...
  /*
    When the orientation changes the container picks up `data-flipping` for
    FLIP_DURATION_MS: the board dips in opacity, and the pieces opt out of
    their own translate transition for the same window (the `transition:
    none` on each piece style below) so nothing slides to its mirror.
  */
  useEffect(() => {
      ...
      */}
      ...
      >
        <div
          ...
          style={{
            /*
             * The flip dip: the board is drawn at reduced opacity for the
             * FLIP_DURATION_MS window and fades back to full. No transform —
             * see the note on FLIP_DURATION_MS for why a rotation was wrong.
             */
            opacity: flipping ? 0.35 : 1,
            transition: `opacity ${FLIP_DURATION_MS}ms ease-out`,
          }}
            ...
            return {
              style: dragging
                ...
                : {
                    ...
                    /*
                     * A flip is a snap, not thirty-two slides through the
                     * centre of the board: while one is in flight the per-piece
                     * transition is suppressed. See FLIP_DURATION_MS.
                     */
                    transition:
