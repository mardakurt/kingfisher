...
export function PrivacyPage(): JSX.Element {
  return (
    ...
    >
      ...
      <p>
        <strong>There is no cloud sync today.</strong> Your work lives on the machine you created
        it on, and Kingfisher does not push it anywhere. To move work between machines, two
        browsers, or a browser and a desktop install:{' '}
        <em>Settings → Database → Export backup</em> on the source machine;{' '}
        <em>Settings → Database → Import backup</em> on the destination machine. The backup
        file is portable JSON, is held on disk under your control, and is never sent to a
        server.
      </p>
      <p>
        A future Kingfisher version may add an opt-in cloud sync. If it does, this page will
        be updated first, the change will be listed in <code>CHANGELOG.md</code>, and the
        sync will be off by default.
      </p>
