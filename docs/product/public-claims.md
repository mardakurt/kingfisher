...
| The current public release is Kingfisher 1.1.0 (web and macOS). | Landing download card, README, install guide       | `package.json` (`"version": "1.1.0"`); `src/release/macos-download.json` names the exact release build offered, its SHA-256 and its trust state. |
