...
| The current public release is Kingfisher 1.2.3 (web and macOS). | Landing download card, README, install guide       | `package.json` (`"version": "1.2.3"`); `src/release/macos-download.json` names the exact release build offered, its SHA-256 and its trust state; `npm run docs:check` asserts every canonical document names the same version.                                           |
