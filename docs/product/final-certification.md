...
| Certification candidate | build 490 (`1.1.0-dev-490`, commit `d36ef00`'s parent tree, dirty) — the packaged gates below were first run on it, then on the release build |
...
| PWA                     | ✅  |  n/a  | `settings.spec.ts` service-worker rows; `manifest.webmanifest` route                                            | installability checked on production                                 | manifest and SW served                      | CERTIFIED                 | —                                                                                                                 |
