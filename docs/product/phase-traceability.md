...
| Updater engine (`electron-updater`), feed in the bundle | `desktop/src/update-service.mjs`, `kingfisher-updater.mjs`      | `electron-updater-import.test.mjs`, `update-protocol.test.mjs`         | `desktop:update:real`; public 1.1.0 → 1.1.1  | Held (repaired in 47) |
...
| Native update window, fifteen states         | `desktop/src/update-window.mjs`, `desktop/src/dialogs/`                      | `update-window.test.mjs`; `desktop:update:dialog`                      | dialog harness         | Held            |
