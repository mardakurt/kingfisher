...
| No account, no cookies, no cross-site tracking; page views counted.  | `default-src 'self'` CSP; the one measurement is Vercel Web Analytics from this origin, disclosed on the privacy page; otherwise only the engines, reference data and Lichess/Chess.com calls the user asked for. |
