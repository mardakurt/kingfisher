...
### BUG-46-14 — A pack installed from a URL was listed twice

**Severity:** Low. `useDataSources` filtered the provider registry by the
_catalog's_ pack ids only, so a pack the catalog does not name — the
`reference-packs` e2e fixture installs one — appeared once as a pack and
again as a provider row labelled "Companion". A statistic labelled with
the wrong source is the class of defect the provenance rule exists to
prevent, even when the numbers happen to agree. **Found by:** the failing
`e2e/reference-packs.spec.ts` (strict-mode violation on the source row).
**Fix:** `0e318fb` — installed sources' ids join the filter.
**Regression:** the same spec, now green.
...
### HARN-46-05 — The Playwright suite had 28 failures nobody had run

`npm run test:e2e` at the Phase 45 handover commit (`b77d3a2`, checked in
a worktree with a cloned `node_modules`) failed 28 of 263: 23 visual
baselines named `*-darwin.png` / `*-linux.png` while the Phase 43 project
rename made Playwright look for `*-chrome-darwin.png`, so every screenshot
comparison failed on "no snapshot" and wrote a file the guard test then
counted; `phase8.spec.ts` clicked a button Phase 41 had renamed; the
reliability spec expected the engine to follow the board through four
moves, which the Phase 30 position guard forbids (Q-46-03); the
reference-packs fixture encoded a hundred games in a one-game pack; and
`showEngineArrows` had no runtime assertion and no `verifiedBy`, which
`settings.spec.ts` treats as a failure by design. Handovers 43–45 each
listed `test:e2e` as green. **Fix:** `0e318fb` — baselines
regenerated through `npm run visual:baselines` and the dead names removed
(the Linux ones can only be regenerated by the `visual-review` workflow,
in `update` mode, and were as dead as the macOS ones); the four specs
corrected to what the product does; `showEngineArrows` asserted at
runtime — an arrow and a legend with it on, neither with it off while the
depth counter still climbs — and shown to fail when the consumer ignores
the preference.
