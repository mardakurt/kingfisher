...
| Firefox | `reliability.spec.ts` (intermittent)                                             | **Application defect, and a larger one beneath it.** Firefox reported both faces as preloaded and unused. Inter is never drawn on a Mac or Windows, the platform face comes first. The mono face was never drawn _anywhere_: `--font-ui` and `--font-code` on `:root` referred to faces defined on `<body>`, so both stacks were invalid and the FEN rendered in the system sans (production, measured) | Inter unpreloaded (`4561a64`); the faces' variables moved to `<html>` so the stacks take effect (`48cc382`, `e2e/typography.spec.ts`, fails before, passes on all three engines); the mono face, now drawn at first paint, stays preloaded. `reliability.spec.ts` 18/18 on Firefox, no warning |
