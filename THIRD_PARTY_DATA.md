...
| -------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `kingfisher-starter`       | Committed, ships with the app                    | The most recent 48 monthly archives, games rated 2200+ (or between titled players), with full scores kept for games rated 2500+ (up to 300 a player) |
...
so; `npm run desktop:smoke -- --packaged` closes it by asking the real probe
helper inside the bundle, and `companion/src/tablebase-fixture.test.mjs` pins
the ten digests so the tables cannot quietly become stubs again — which they
did, for one commit, while this section went on describing a verified set.
...
| Verified        | SHA-256 of all ten files against the publisher's own `sha256` manifest at `tables/standard/sha256`; re-verified 2026-09-12          |
