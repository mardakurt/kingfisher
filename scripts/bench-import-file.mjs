...
import { benchmark, equivalenceSample, gb, mb, moveSearchBenchmark, n } from './bench-real-scale.mjs';
import { closeApp } from './load-app.mjs';

const args = { file: null, out: null, workers: undefined, queryOnly: null, sample: 30_000 };
...
if (args.queryOnly) {
  ...
  stats = await runImport(
    database,
    { file: args.file, ...(args.workers ? { workers: args.workers } : {}), licence: 'Lichess database, CC0' },
    (state) => {
  ...
  console.log(`wall time          ${(stats.elapsedMs / 60000).toFixed(1)} min (of which indexes ${(stats.indexMs / 60000).toFixed(1)} min)`);
  console.log(`throughput         ${Math.round(stats.imported / (stats.elapsedMs / 1000))} games/s`);
  console.log(`peak memory (rss)  ${mb(stats.peakRssBytes)}`);
}
const size = statSync(file).size;
console.log(`database on disk   ${gb(size)} · ${n(database.count())} games · ${Math.round(size / database.count())} bytes a game`);
console.log('\n--- queries (ms) ----------------------------------------------');
