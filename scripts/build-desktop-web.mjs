...
const EXCLUDED = [
  ...
  /*
    The full-network browser Stockfish, 113 MB a build, which the web
    deployment installs (`npm run engine:install -- --full`) so a person with
    no companion can analyse with the same evaluation network the native
    binary runs. The Mac application ships native Stockfish 19; a second copy
    of the network in WebAssembly would double the download for a build the
    desktop never needs to offer. The staged manifest is rewritten below so
    the desktop's engine list does not name a build that is not in the bundle.
  */
  {
    at: 'engine/stockfish/stockfish-18.js',
    because: 'full-network browser Stockfish; the Mac app has native Stockfish',
  },
  {
    at: 'engine/stockfish/stockfish-18.wasm',
    because: 'full-network browser Stockfish; the Mac app has native Stockfish',
  },
  {
    at: 'engine/stockfish/stockfish-18-single.js',
    because: 'full-network browser Stockfish; the Mac app has native Stockfish',
  },
  {
...
/*
  The browser-engine manifest, minus the builds that were just left out. A
  manifest naming a `full` build the bundle does not carry would put an engine
  in the desktop's selector that fails on first use — the dead control the
  registry exists to prevent.
*/
const stagedManifest = path.join(OUT, 'public', 'engine', 'stockfish', 'manifest.json');
if (existsSync(stagedManifest)) {
  const manifest = JSON.parse(readFileSync(stagedManifest, 'utf8'));
  const builds = (manifest.builds ?? []).filter((build) => build.network !== 'full');
  if (builds.length !== (manifest.builds ?? []).length) {
    writeFileSync(stagedManifest, `${JSON.stringify({ ...manifest, builds }, null, 2)}\n`);
    console.log('  (browser-engine manifest rewritten without the full-network builds)');
  }
}
