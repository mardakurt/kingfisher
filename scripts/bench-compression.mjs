 ...
 * phase.
 */
import { gzipSync, brotliCompressSync, brotliDecompressSync, gunzipSync, constants as zlibConstants } from 'node:zlib';
...
function makeInput(lines) {
  return lines.map((line) => row(line.key, line.san, line.uci, line.games, line.white, line.draws, line.black, line.avgRating, line.lastYear)).join('\n');
}
...
if (file) {
  ...
  for (let i = 0; i < 100; i += 1) {
    ...
    for (let m = 0; m < 30; m += 1) {
      moves.push({ san: `e${m}`, uci: `e${m}e${m + 1 || 1}`, games: 100 - m, white: 50, draws: 30, black: 20, avgRating: 2400, lastYear: 2024 });
    }
...
const brotli = brotliCompressSync(Buffer.from(input, 'utf8'), { params: { [zlibConstants.BROTLI_PARAM_QUALITY]: 11 } });

console.log('');
console.log(`Codec          Bytes      Ratio`);
console.log(`-------------- ---------- ------`);
console.log(`plain          ${String(input.length).padStart(10)}  ${(input.length / input.length).toFixed(3)}`);
console.log(`gzip -9        ${String(gz.byteLength).padStart(10)}  ${(gz.byteLength / input.length).toFixed(3)}`);
console.log(`brotli q11     ${String(brotli.byteLength).padStart(10)}  ${(brotli.byteLength / input.length).toFixed(3)}`);
console.log('');
console.log('Save ratio over gzip:', `${(((gz.byteLength - brotli.byteLength) / gz.byteLength) * 100).toFixed(2)}%`);

// Decompression round-trip check
const round = gunzipSync(gz);
const rtBrotli = brotliDecompressSync(brotli);
console.log('Round-trip ok:', round.toString('utf8') === input && rtBrotli.toString('utf8') === input);
