...
> **Public release · 1.3.1** (web · macOS, Developer ID signed and notarised)
...
mirror. The site counts page views and load times with cookieless Vercel Web Analytics and Speed Insights; see
...
|                       |                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Engine**            | Stockfish 18, as WebAssembly in a Web Worker, lite or full network. Sandboxed by the browser.                                                                                                                                                                                                                                                                                                          |
| **Opening Explorer**  | 206,451 broadcast games between rated or titled players (including online events), 2022–2026, as 300,413 position aggregates indexed through **twenty full moves**. On your machine, and it answers with the network off.                                                                                                                                                                              |
...
| **Games**             | 38,749 full elite game scores, openable on the board.                                                                                                                                                                                                                                                                                                                                                  |
| **Players**           | 13,738 player identities from the reference, plus a curated roster of 106 historical figures — the whole championship lineage from Steinitz to Gukesh, the women's lineage from Menchik to Ju Wenjun, and twenty-five players from before FIDE existed.                                                                                                                                                |
...
the current public release is **Kingfisher 1.3.1** (web and macOS), and
...
| Engines              | Stockfish 18 in a Worker and the managed native fleet through the companion. MultiPV 1–5, live depth / nodes / nps, evaluation bar, stability and line-separation metrics, persistent deep analysis, labelled Monte Carlo playouts, and engine evidence exchanged as provenance-preserving files                                                                                                                     |
...
| Databases            | IndexedDB for everyday collections; optional SQLite through the companion. Large PGN, compressed PGN and ChessBase files stream into SQLite without entering browser memory; a compact line index answers material, theme and route searches. Both stores use the same fingerprints and canonical position keys                                                                                                      |
...
npm run desktop:walk       # a seeded random walk through the packaged application, with invariants
npm run desktop:soak       # the same walk for thirty minutes, sampling memory and processes
npm run desktop:certify    # the whole packaged gate, one command
npm run desktop:public:verify -- --full   # the DMG the public downloads, byte for byte
```
...
| Check for Updates                   | **yes** — on the click only; `npm run desktop:update:real` performed a real internal 1.0.5 → 1.1.0 staging update, and Phase 49 the real public 1.1.0 → 1.1.1 update, through the menu and dialog, with the study still there afterwards |
...
into `public/engine/stockfish/`; `-- --full` adds the two full-network builds
(226 MB), which the web deployment installs and the Mac build leaves out. They are **not** committed and **not** an npm
